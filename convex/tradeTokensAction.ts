...
export const getOrCreateTradeToken = internalAction({
    ...
    handler: async (ctx, args): Promise<Doc<'tradeTokens'> | null> => {
        const { address } = args;
        const tradeToken: Doc<'tradeTokens'> | null = await ctx.runQuery(internal.tradeTokens.get, { address });
        ...
        const newTradeToken: Doc<'tradeTokens'> | null = await ctx.runQuery(internal.tradeTokens.get, { address });
